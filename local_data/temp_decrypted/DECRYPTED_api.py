...
# [MODIFIKASI DATABASE]
# Kita siapkan path untuk dua database berbeda
user_db_path = os.path.join(base_dir, 'users.db')      # Untuk akun & auth
msg_db_path = os.path.join(base_dir, 'messages.db')    # Untuk chat history
...
# [MODIFIKASI KONFIGURASI SQLALCHEMY]
# 1. URI Default (akan digunakan oleh tabel tanpa bind_key, yaitu User)
app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{user_db_path}'

# 2. Binds (Mapping untuk database tambahan)
...
class User(db.Model):
    # Model ini TIDAK punya __bind_key__, jadi masuk ke database DEFAULT (users.db)
...
class Message(db.Model):
    # [PENTING] Model ini diarahkan ke database 'messages' (messages.db)
...
<body><div class="overlay"><h1>API Berjalan</h1><p>Layanan Kriptografi & Multi-DB Aktif.</p></div></body>
</html>
"""
...
with app.app_context():
    ...
    # [PENTING] create_all() otomatis membuat tabel di KEDUA database
    # berdasarkan konfigurasi bind_key
    db.create_all()
